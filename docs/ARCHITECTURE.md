...
El modulo `CashRegister` es la capa de cajas fisicas, apertura, movimientos, arqueo y cierre de caja. Una caja fisica pertenece a una sucursal y puede tener muchas sesiones historicas, pero solo una sesion abierta a la vez. Una sesion de caja pertenece a una caja fisica opcional, a una sucursal y a un cajero. Los movimientos pueden estar en `USD` o `VES`, guardan snapshot de tasa cuando aplica y actualizan el monto esperado. El cierre compara monto contado contra monto esperado y guarda la diferencia. POS se integra con caja asociando cada checkout a una sesion abierta y registrando pagos capturados como movimientos `pos_payment`, sin convertir el cierre de caja en responsabilidad de POS.
