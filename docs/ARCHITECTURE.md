...
El modulo `CashRegister` es la capa de cajas fisicas, apertura, movimientos, arqueo y cierre de caja. Una caja fisica pertenece a una sucursal y puede tener muchas sesiones historicas, pero solo una sesion abierta a la vez. Una sesion de caja pertenece a una caja fisica, a una sucursal y a un cajero para operar POS. Los movimientos pueden estar en `USD` o `VES`, guardan snapshot de tasa cuando aplica y actualizan el monto esperado. El cierre compara monto contado contra monto esperado y guarda la diferencia. POS se integra con caja asociando cada checkout a una sesion abierta con caja fisica activa y registrando pagos capturados como movimientos `pos_payment`, sin convertir el cierre de caja en responsabilidad de POS.

Varias cajas pueden operar al mismo tiempo. La independencia se mantiene porque cada orden POS apunta a una `cash_register_session_id` especifica y cada pago capturado registra movimiento en esa misma caja. El backend impide que dos cajeros abran la misma caja fisica al mismo tiempo y tambien impide que un mismo cajero tenga dos sesiones abiertas. La competencia por inventario se resuelve en `Sales` e `Inventory`: si dos cajas intentan vender la ultima unidad, la primera confirmacion descuenta el stock y la segunda falla por stock insuficiente sin dejar stock negativo ni movimientos de caja permanentes. En productos serializados, la confirmacion bloquea los IMEIs seleccionados y solo una venta puede marcar la unidad como `sold`.
...
El modulo `Suppliers` mantiene los proveedores por empresa. El modulo `Purchases` crea documentos de compra primero como `draft`; en esa fase no mueve inventario. La compra guarda numero de factura/documento del proveedor, fecha de emision y vencimiento. La recepcion puede ser parcial o total; cada recepcion genera movimientos `purchase` mediante `InventoryMovementService`, acumula `received_quantity` por item y recalcula los montos recibidos. Si el producto es serializado, cada IMEI o serial recibido se crea en `product_units` como unidad disponible y enlazada al movimiento de compra.
