<?php

namespace Tests\Feature\AdminPortal;

use Tests\TestCase;

class AdminPortalWebTest extends TestCase
{
    public function test_admin_portal_page_is_available(): void
    {
        $this
            ->get('/admin')
            ->assertOk()
            ->assertSee('Portal administrativo')
            ->assertSee('admin-login-form')
            ->assertSee('admin-tenant-switcher')
            ->assertSee('Productos y precios')
            ->assertSee('admin-inventory-module')
            ->assertSee('admin-inventory-new')
            ->assertSee('admin-inventory-search')
            ->assertSee('admin-inventory-active')
            ->assertSee('admin-inventory-quick-status')
            ->assertSee('admin-inventory-filter-summary')
            ->assertSee('admin-inventory-name')
            ->assertSee('admin-inventory-sku')
            ->assertSee('admin-inventory-tracking-edit')
            ->assertSee('admin-inventory-rate-type')
            ->assertSee('admin-inventory-warranty')
            ->assertSee('admin-inventory-active-edit')
            ->assertSee('admin-inventory-deactivate')
            ->assertSee('admin-inventory-detail')
            ->assertSee('admin-inventory-detail-stock')
            ->assertSee('admin-inventory-detail-prices')
            ->assertSee('admin-inventory-detail-changes')
            ->assertSee('admin-inventory-table')
            ->assertSee('Precios por lista')
            ->assertSee('admin-price-list-rows')
            ->assertSee('admin-price-list-save')
            ->assertSee('admin-price-copy-base')
            ->assertSee('Historial de movimientos')
            ->assertSee('admin-movements-module')
            ->assertSee('admin-movements-search')
            ->assertSee('admin-movements-table')
            ->assertSee('Proveedores')
            ->assertSee('admin-suppliers-module')
            ->assertSee('admin-suppliers-search')
            ->assertSee('admin-suppliers-table')
            ->assertSee('admin-supplier-name')
            ->assertSee('admin-supplier-document-type')
            ->assertSee('admin-supplier-save')
            ->assertSee('admin-supplier-deactivate')
            ->assertSee('Ordenes de compra')
            ->assertSee('admin-purchases-module')
            ->assertSee('admin-purchases-search')
            ->assertSee('admin-purchases-status-filter')
            ->assertSee('admin-purchases-supplier-filter')
            ->assertSee('admin-purchases-table')
            ->assertSee('admin-purchase-supplier')
            ->assertSee('admin-purchase-product')
            ->assertSee('admin-purchase-warehouse')
            ->assertSee('admin-purchase-save')
            ->assertSee('admin-purchase-receive')
            ->assertSee('Cuentas por cobrar')
            ->assertSee('data-portal-section="receivables"', false)
            ->assertSee('admin-receivables-module')
            ->assertSee('admin-receivables-search')
            ->assertSee('admin-receivables-status-filter')
            ->assertSee('admin-receivables-customer-filter')
            ->assertSee('admin-receivables-table')
            ->assertSee('admin-receivable-payment-amount')
            ->assertSee('admin-receivable-fill-balance')
            ->assertSee('admin-receivable-collect')
            ->assertSee('Clientes')
            ->assertSee('data-portal-section="customers"', false)
            ->assertSee('admin-customers-module')
            ->assertSee('admin-customers-search')
            ->assertSee('admin-customers-active')
            ->assertSee('admin-customers-type')
            ->assertSee('admin-customers-table')
            ->assertSee('admin-customer-name')
            ->assertSee('admin-customer-document-type')
            ->assertSee('admin-customer-document-number')
            ->assertSee('admin-customer-generic-edit')
            ->assertSee('admin-customer-active-edit')
            ->assertSee('admin-customer-save')
            ->assertSee('admin-customer-deactivate')
            ->assertSee('Cuentas por pagar')
            ->assertSee('data-portal-section="payables"', false)
            ->assertSee('admin-payables-module')
            ->assertSee('admin-payables-search')
            ->assertSee('admin-payables-status-filter')
            ->assertSee('admin-payables-supplier-filter')
            ->assertSee('admin-payables-table')
            ->assertSee('admin-payable-payment-amount')
            ->assertSee('admin-payable-fill-balance')
            ->assertSee('admin-payable-pay')
            ->assertSee('Usuarios y permisos')
            ->assertSee('admin-users-module')
            ->assertSee('admin-access-users-table')
            ->assertSee('admin-access-roles-table')
            ->assertSee('admin-access-permissions-grid')
            ->assertSee('data-access-tab="users"', false)
            ->assertSee('data-access-tab="profiles"', false)
            ->assertSee('data-access-tab="permissions"', false)
            ->assertSee('Perfil Cajero')
            ->assertSee('Perfil Inventario')
            ->assertSee('Perfil Gerente')
            ->assertSee('Reportes operativos')
            ->assertSee('data-portal-section="reports"', false)
            ->assertSee('admin-reports-module')
            ->assertSee('admin-reports-refresh')
            ->assertSee('admin-reports-date-from')
            ->assertSee('admin-reports-date-to')
            ->assertSee('admin-reports-branch')
            ->assertSee('admin-reports-cash-register')
            ->assertSee('admin-reports-cashier')
            ->assertSee('admin-reports-order-status')
            ->assertSee('admin-reports-clear-filters')
            ->assertSee('admin-reports-export-orders')
            ->assertSee('admin-reports-export-payments')
            ->assertSee('admin-reports-export-products')
            ->assertSee('admin-reports-export-cash')
            ->assertSee('admin-reports-orders-table')
            ->assertSee('admin-reports-payments-table')
            ->assertSee('admin-reports-products-table')
            ->assertSee('admin-reports-cash-table');
    }
}
