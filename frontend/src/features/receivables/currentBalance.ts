...
export function activeUsdVesRate(rates: CurrentExchangeRate[]): CurrentExchangeRate | null {
  return rates.find((rate) => rate.is_active !== false && rate.base_currency === 'USD' && rate.quote_currency === 'VES') ?? null;
}

export function currentLocalBalance(balance: BaseBalance, rate: CurrentExchangeRate | null): number | null {
