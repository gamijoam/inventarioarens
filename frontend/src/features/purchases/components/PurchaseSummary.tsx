 ...
 */
import { Building2, Calendar, FileText, Hash, TrendingUp } from 'lucide-react';
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="space-y-1">
            ...
            <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-text-muted">
              ...
              {purchase.created_at && (
                <span>Creada {formatRelative(purchase.created_at)}</span>
              )}
            </div>
          </div>
          ...
          <div className="flex items-end gap-4">
            ...
            {totalLocal > 0 && totalLocal !== totalBase && (
              <div className="text-right">
                <div className="text-xs uppercase tracking-wide text-text-muted">Total VES</div>
                <div className="text-2xl font-bold tabular-nums text-text-muted">
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          ...
          <InfoRow icon={<TrendingUp className="size-4" />} label="Tasa de cambio">
            ...
            {purchase.exchange_rate_type_code && (
              <span className="ml-1 text-text-muted">({purchase.exchange_rate_type_code})</span>
            )}
                  ...
                  <li key={it.id} className="flex items-center justify-between gap-3 px-3 py-2 text-sm">
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="flex items-center gap-1.5 text-xs text-text-muted">
                        {product?.sku && (
                          <code className="rounded bg-bg px-1 py-0.5">{product.sku}</code>
                        )}
                        <span>|</span>
                        <span>
                          {warehouse?.code ?? `Almacen #${it.warehouse_id}`}
                        </span>
                      </div>
                    </div>
                    <div className="flex items-center gap-3 text-right">
                      ...
                      {cost != null && (
                        <div className="tabular-nums text-text-muted">
                          ...
                          <div className="text-[10px] uppercase tracking-wide">c/u</div>
                        </div>
                      )}
                    </div>
                  </li>
