...
export const StorePromotionSchema = z.object({
  ...
  if (promotion.benefit_type === 'fixed_bundle_price' && promotion.items.length < 2) {
    context.addIssue({ code: z.ZodIssueCode.too_small, minimum: 2, inclusive: true, type: 'array', path: ['items'] });
  }
  if (promotion.benefit_type === 'percent_discount' && promotion.discount_percent == null) {
    context.addIssue({ code: z.ZodIssueCode.custom, path: ['discount_percent'], message: 'El porcentaje es obligatorio.' });
  }
  if (promotion.benefit_type === 'fixed_discount' && promotion.discount_amount_usd == null) {
    context.addIssue({ code: z.ZodIssueCode.custom, path: ['discount_amount_usd'], message: 'El monto fijo es obligatorio.' });
  }
  if (promotion.benefit_type === 'buy_x_get_y') {
    ...
    if (!roles.includes('trigger') || !roles.includes('reward')) {
      context.addIssue({ code: z.ZodIssueCode.custom, path: ['items'], message: 'Debe definir componentes trigger y reward.' });
    }
  }
  if (['fixed_item_price', 'fixed_bundle_price'].includes(promotion.benefit_type) && promotion.price_usd == null) {
    context.addIssue({ code: z.ZodIssueCode.custom, path: ['price_usd'], message: 'El precio USD es obligatorio.' });
  }
});
