...
def resolve_project_path_unrestricted(settings: Settings, relative_path: str) -> Path:
    if not relative_path or relative_path.startswith(("/", "\\")):
        raise ValueError("Usa una ruta relativa dentro del proyecto.")
    normalized = relative_path.replace("\\", "/").strip()
    if not SAFE_PATH_RE.match(normalized):
    ...
    if any(part == ".." for part in parts):
        raise ValueError("No se permite subir de directorio.")
    path = (settings.project_root / "/".join(parts)).resolve()
    if settings.project_root not in (path, *path.parents):
        raise ValueError("La ruta salio del proyecto permitido.")
    return path
...
def create_mcp():
    ...
    @mcp.tool()
    def run_graphify(access_key: str, command: str) -> dict[str, Any]:
        """
        Ejecuta comandos Graphify permitidos.
        """
        require_access(settings, access_key)
        ...
        if command.startswith("query "):
            args = ["graphify", "query", command[6:]]
        elif command.startswith("path "):
            args = ["graphify", "path"] + command[5:].split(" ")
        elif command.startswith("explain "):
            args = ["graphify", "explain", command[8:]]
        elif command == "update":
            args = allowed["update"]
        else:
            raise ValueError(
                "Comando graphify no permitido. Usa query/path/explain/update."
            )
    ...
    @mcp.tool()
    def artisan(access_key: str, command: str) -> dict[str, Any]:
        """
        Ejecuta comandos Artisan Laravel.
        """
        require_access(settings, access_key)
        ...
        if any(x in command for x in blocked):
            raise ValueError(
                "Comando Artisan bloqueado por seguridad."
            )
    ...
    @mcp.tool()
    def run_phpunit(access_key: str, path: str = "") -> dict[str, Any]:
        """
        Ejecuta PHPUnit.
        """
        require_access(settings, access_key)
    ...
    @mcp.tool()
    def git_command(access_key: str, command: str) -> dict[str, Any]:
        """
        Operaciones git de solo lectura.
        """
        require_access(settings, access_key)
        ...
        if action not in allowed:
            raise ValueError(
                "Solo permitido: status, diff, log, branch"
            )
    ...
    @mcp.tool()
    def run_pint(access_key: str) -> dict[str, Any]:
        """
        Ejecuta Laravel Pint.
        """
        require_access(settings, access_key)
    ...
    @mcp.tool()
    def admin_shell(access_key: str, command: str, cwd: str = ".", timeout_seconds: int = 120) -> dict[str, Any]:
        """
        Ejecuta cualquier comando shell dentro del proyecto INVENTARIOARENS.

        Requiere access_key. Es una herramienta administrativa poderosa: puede
        modificar archivos, instalar dependencias o ejecutar scripts del repo.
        El cwd siempre queda limitado a /opt/inventarioarens-cloud.
        """
        require_access(settings, access_key)
        workdir = resolve_project_path_unrestricted(settings, cwd)
        result = subprocess.run(
            command,
            cwd=workdir,
            capture_output=True,
    ...
    @mcp.tool()
    def admin_artisan(access_key: str, command: str, timeout_seconds: int = 300) -> dict[str, Any]:
        """
        Ejecuta cualquier comando Artisan sin lista de bloqueo.

        Requiere access_key. Usar con cuidado en produccion: comandos como
        migrate:fresh, db:wipe o key:generate son destructivos.
        """
        require_access(settings, access_key)
        result = subprocess.run(
            ["php", "artisan"] + command.split(),
    ...
    @mcp.tool()
    def admin_git(access_key: str, command: str, timeout_seconds: int = 120) -> dict[str, Any]:
        """
        Ejecuta cualquier comando git dentro del repo.

        Requiere access_key. No limita acciones: puede hacer add, commit,
        fetch, pull, push, checkout, restore, etc.
        """
        require_access(settings, access_key)
        result = subprocess.run(
            ["git"] + command.split(),
    ...
    @mcp.tool()
    def db_execute(access_key: str, sql: str) -> dict[str, Any]:
        """
        Ejecuta SQL arbitrario sobre inventory_arens.

        Requiere access_key. Permite SELECT, INSERT, UPDATE, DELETE, DDL, etc.
        Usar con cuidado: no hay modo read-only ni rollback automatico.
        """
        require_access(settings, access_key)
        with connect_db(settings) as conn:
            with conn.cursor() as cur:
    ...
    @mcp.tool()
    def read_any_project_file(access_key: str, path: str, max_bytes: int = 200000) -> dict[str, Any]:
        """
        Lee cualquier archivo dentro del proyecto, incluyendo .env, storage y vendor.
        Requiere access_key.
        """
        require_access(settings, access_key)
        target = resolve_project_path_unrestricted(settings, path)
        if not target.is_file():
            raise FileNotFoundError(path)
        size = target.stat().st_size
        read_size = min(max(1, int(max_bytes)), 2_000_000)
        data = target.read_bytes()[:read_size]
        return {
            "path": target.relative_to(settings.project_root).as_posix(),
            "size": size,
    ...
    @mcp.tool()
    def write_any_project_file(access_key: str, path: str, content: str) -> dict[str, Any]:
        """
        Escribe cualquier archivo dentro del proyecto, incluyendo .env y storage.
        Requiere access_key.
        """
        require_access(settings, access_key)
        target = resolve_project_path_unrestricted(settings, path)
    ...
    def patch_project_file(
    	...
    	"""
    	Reemplaza un bloque exacto dentro de un archivo existente.
    	"""
    	require_access(settings, access_key)
    	...
    	if old_text not in current:
        	raise RuntimeError(
            	"No se encontró el bloque a reemplazar"
        	)
