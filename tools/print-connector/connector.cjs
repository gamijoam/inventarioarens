...
const DEFAULT_DATA_DIR = path.join(process.env.ProgramData || path.join(os.homedir(), 'InventarioArens'), 'PrintConnector');
...
async function readResponse(response) {
  ...
  if (!response.ok) {
    throw makeError(body?.message || `Cloud API respondio ${response.status}`, response.status, body);
  }
  return body;
}
...
function buildPlainTicket(ticket) {
  ...
  if (profile.show_cash_register !== false && order.cash_register_name) push(`Caja: ${order.cash_register_name}`);
  if (profile.show_branch !== false && order.branch_name) push(`Sucursal: ${order.branch_name}`);
  if (profile.show_customer !== false) push(`Cliente: ${order.customer_name || 'Consumidor Final'}`);

  push('-'.repeat(max));
  for (const item of ticket?.items || []) {
    push(item.product_name || 'Producto');
    if (profile.show_item_sku !== false && item.sku) push(item.sku);
    push(`${item.quantity || 0} x $${Number(item.unit_price || 0).toFixed(2)}  $${Number(item.total || 0).toFixed(2)}`);
  ...
  if (profile.show_total_local !== false) push(`Total VES: Bs ${Number(totals.total_local_amount || 0).toFixed(2)}`);
  ...
  for (const payment of ticket?.payments || []) {
    ...
    if (profile.show_payment_reference !== false && payment.reference) push(`Ref: ${payment.reference}`);
  }
...
function sendTcp(host, port, buffer, timeoutMs = 5000) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => socket.destroy(makeError(`Timeout conectando a ${host}:${port}`)), timeoutMs);
...
class PrintConnector {
  constructor({ config, fetchImpl = globalThis.fetch, printImpl, saveConfig, sleep, onError, exec = execFileAsync } = {}) {
    this.config = { ...config };
  ...
  async register({ code, name, installationId, version, metadata } = {}) {
    const response = await this.fetchImpl(apiUrl(this.config.cloudApiUrl, '/printing/connectors/register'), {
      method: 'POST',
      headers: { Accept: 'application/json', 'Content-Type': 'application/json' },
      body: JSON.stringify({ code, name, installation_id: installationId, version, metadata }),
    });
  ...
  async pollOnce() {
    const body = await this.request('GET', `/printing/connector/jobs?limit=${this.config.batchSize || 20}`);
    const jobs = body.data || [];
    const results = [];
    for (const job of jobs) results.push(await this.processJob(job));
    if (Date.now() - this.lastHeartbeatAt >= (this.config.heartbeatIntervalMs || 60_000)) await this.heartbeat();
    return results;
  }
  ...
  async processJob(summary) {
    let claim;
    try {
      const body = await this.request('POST', `/printing/connector/jobs/${encodeURIComponent(summary.job_uuid)}/claim`);
      claim = body.data;
    } catch (error) {
      if (error.status === 404 || error.status === 409) return { jobUuid: summary.job_uuid, status: 'skipped', reason: error.message };
      throw error;
    }

    try {
      await this.printImpl(claim.job);
      await this.request('POST', `/printing/connector/jobs/${encodeURIComponent(summary.job_uuid)}/ack`, {
      ...
      return { jobUuid: summary.job_uuid, status: 'failed', message: error.message };
    }
  }
  ...
  async printJob(job, exec = execFileAsync) {
    ...
    if (station.printer_type === 'network') {
      if (!station.network_host) throw makeError('La estacion de red no tiene network_host configurado.');
      await sendTcp(
        station.network_host,
        Number(station.network_port || 9100),
        buildEscPos(text, { cutPaper: profile.cut_paper, openCashDrawer: profile.open_cash_drawer }),
        5000,
      );
      return;
    }
...
async function saveConfigFile(config, filePath = path.join(config.dataDir || DEFAULT_DATA_DIR, 'config.json')) {
...
async function main(argv = process.argv.slice(2)) {
  ...
  const configPath = process.env.PRINT_CONNECTOR_CONFIG || path.join(DEFAULT_DATA_DIR, 'config.json');
  ...
  if (command === 'status') {
    process.stdout.write(`${JSON.stringify({ ...config, token: config.token ? '[configured]' : null }, null, 2)}\n`);
    return;
  }
  if (command === 'register') {
    ...
    await connector.register({ code, installationId, name, version: process.env.PRINT_CONNECTOR_VERSION || '0.1.0' });
