...
const { PrintConnector, buildEscPos, buildPlainTicket } = require('./connector.cjs');
...
function job(uuid = 'job-1') {
  return {
    ...
    payload_snapshot: {
      ...
      profile: { paper_width_mm: 80, logo_text: 'Tienda', cut_paper: true },
      pos_order: { id: 10, sale_id: 20, customer_name: 'Cliente' },
      items: [{ product_name: 'Producto', quantity: 1, unit_price: 5, total: 5 }],
...
test('polls, claims, prints and acknowledges a job with the connector token', async () => {
  ...
  const fetchImpl = async (url, options) => {
    ...
    if (url.endsWith('/printing/connector/jobs/job-1/claim')) return response(200, { data: { claim_token: 'claim-1', job: job() } });
    if (url.endsWith('/printing/connector/jobs/job-1/ack')) return response(200, { data: { status: 'printed' } });
    if (url.endsWith('/printing/connector/heartbeat')) return response(200, { data: { connector: { status: 'active' } } });
    throw new Error(`Unexpected URL ${url}`);
  };
  const connector = new PrintConnector({
    config: { cloudApiUrl: 'https://app.example.test/api', token: 'secret', batchSize: 20, heartbeatIntervalMs: 999999 },
    fetchImpl,
  ...
  assert.equal(calls[2].options.body, JSON.stringify({ claim_token: 'claim-1', status: 'printed' }));
});
...
test('registers once with a pairing code and persists the issued token', async () => {
  ...
  const connector = new PrintConnector({
    ...
    saveConfig: async (value) => { saved = value; },
  });

  await connector.register({ code: 'PAIR-1', name: 'Caja', installationId: 'install-1', version: '0.1.0' });
...
test('acknowledges a failed print so the cloud queue can retry it later', async () => {
  ...
  const fetchImpl = async (url, options) => {
    if (url.endsWith('/printing/connector/jobs?limit=20')) return response(200, { data: [job('job-2')] });
    if (url.endsWith('/printing/connector/jobs/job-2/claim')) return response(200, { data: { claim_token: 'claim-2', job: job('job-2') } });
  ...
  const connector = new PrintConnector({
    config: { cloudApiUrl: 'https://app.example.test/api', token: 'secret' },
    fetchImpl,
    printImpl: async () => { throw new Error('Impresora apagada'); },
  });

  const result = await connector.pollOnce();

  assert.equal(result[0].status, 'failed');
  assert.deepEqual(acknowledgements, [{ claim_token: 'claim-2', status: 'failed', message: 'Impresora apagada' }]);
});
...
test('keeps polling after a temporary cloud error', async () => {
  ...
  const connector = new PrintConnector({
    config: { cloudApiUrl: 'https://app.example.test/api', token: 'secret' },
    fetchImpl: async () => { throw new Error('Cloud offline'); },
