...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (value === '--output-dir') args.outputDir = path.resolve(argv[++index]);
...
async function createSeaExecutable({ outputDir, version }) {
  ...
  await fs.writeFile(path.join(stageDir, 'VERSION.txt'), `${version}\n`, 'ascii');
  await fs.copyFile(path.join(ROOT, 'install-task.ps1'), path.join(stageDir, 'install-task.ps1'));
  await fs.copyFile(
    path.join(ROOT, 'uninstall-task.ps1'),
